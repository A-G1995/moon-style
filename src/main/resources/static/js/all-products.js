// /js/all-products.js
(function () {
    if (window.__allProductsBooted) return; window.__allProductsBooted = true;

    // ✅ اگر ادمین است، نگذاریم این صفحه را ببیند
    const isAdmin = localStorage.getItem('isAdmin') === 'true';
    if (isAdmin) {
        location.replace('/admin.html'); // استفاده از replace تا نتواند back بزند
        return;
    }
    ...
    function renderProducts(items) {
        grid.innerHTML = ''; // clear when ready
        if (!items.length) {
    ...
    async function load() {
        ...
        try {
            ...
            if (!Array.isArray(items)) throw new Error('ساختار داده نامعتبر است.');
